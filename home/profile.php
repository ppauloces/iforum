...
foreach($resBuscaAluno1 as $alunoInfo){
   ...
   <!doctype html>
      ...
      <body>
         ...
         <?php 
         $buscafoto = $pdo->prepare('SELECT * FROM alunos WHERE id_aluno = :id_aluno');
         $buscafoto->bindParam(':id_aluno', $_GET['id'], PDO::PARAM_STR);
         $buscafoto->execute();
         $resBuscaFoto = $buscafoto->fetch(PDO::FETCH_ASSOC);
         $rowBuscaFoto = $buscafoto->rowCount();
         
         if($resBuscaFoto['foto_perfil']==""){
            $foto = "padrao.png";
         }else{

            $foto = $resBuscaFoto['foto_perfil'];
         }
