...
$amizade = $pdo->prepare("SELECT * FROM amizade WHERE id_aluno_para=:id_aluno_para AND id_aluno_de=:id_aluno_de AND situacao = 0");
		$amizade->execute([
			'id_aluno_de' => $ele,
		...
		echo $row_verifica_amzd['situacao'];
		$id_amzd = $row_verifica_amzd['id_amzd'];

		if($row_verifica_amzd['situacao'] == 0){
